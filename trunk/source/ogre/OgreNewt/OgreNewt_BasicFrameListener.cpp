...
namespace OgreNewt
{
	...
	BasicFrameListener::BasicFrameListener( Ogre::RenderWindow* win, Ogre::SceneManager* mgr, OgreNewt::World* W, GameServices *gs, int update_framerate) :
		FrameListener()
{
	m_World = W;
...
bool BasicFrameListener::frameStarted(const Ogre::FrameEvent &evt)
{
	...
	if ((m_elapsed > m_update) && (m_elapsed < (m_update * 10)) )
	{
		...
		{
			GameEvent *evnt = new GameEvent("PhysicsUpdateStarted");
			mGameServices->events()->throwEvent(evnt);

			m_World->update( m_update );

			evnt = new GameEvent("PhysicsUpdateEnded");
			mGameServices->events()->throwEvent(evnt);
